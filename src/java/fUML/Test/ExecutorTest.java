...
public class ExecutorTest extends fUML.Test.Test {
    ...
    public void testEvaluate(String typeName) {
        ...
        Debug
                .println("[testEvaluate] Value specification is "
    ...
    public void testExecute(String behaviorName) {
        ...
        if ((element == null) || !(element instanceof Behavior)) {
            Debug.println("[testExecute] " + behaviorName + " not found or is not an behavior.");
            return;
        }
        ...
        for (int i = 0; i < outputParameterValues.size(); i++) {
            ...
            Debug.println("[testExecute] Output parameter " + outputParameterValue.parameter.name
    ...
    public void testStart(String typeName) {
        ...
        for (int i = 0; i < outputParameterValues.size(); i++) {
            ParameterValue outputParameterValue = outputParameterValues.getValue(i);
            Debug.println("[testStart] Output parameter " + outputParameterValue.parameter.name
                    + " has " + outputParameterValue.values.size() + " value(s):");
            for (int j = 0; j < outputParameterValue.values.size(); j++) {
