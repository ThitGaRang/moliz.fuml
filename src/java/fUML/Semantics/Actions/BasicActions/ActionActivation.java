...
public abstract class ActionActivation extends
    ...
    public void fire(fUML.Semantics.Activities.IntermediateActivities.TokenList incomingTokens) {
        // Do the main action behavior then oncurrently fire all output pin
        // activations and offer a single control token.
        ...
        // Activate the action again, if tokens have been left on input pins and
        // the action has no incoming control flows.
        ...
        if (((Action) (this.node)).input.size() > 0 & this.node.incoming.size() == 0) {
            ...
            while (fireAgain & j <= inputPins.size()) {
                ...
                fireAgain = inputPinActivation.isReady()
                        & inputPinActivation.countUnofferedTokens() > 0;
                j = j + 1;
            }
    ...
    public boolean isReady() {
        // In addition to the default condition, check that the sources of all
        // incoming edges (control flows) have offers and all input pin
        // activations are ready.
    ...
    public void sendOffers() {
        ...
        // *** Fire all output pins concurrently. ***
     ...
     * @generated
     */
    public boolean valueParticipatesInLink(fUML.Semantics.Classes.Kernel.Value value,
