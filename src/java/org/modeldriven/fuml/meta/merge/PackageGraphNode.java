...
public class PackageGraphNode {
    		...
    		PackageGraphNode source, Map<PackageGraphNode, PackageGraphNode> visited)
    {
        if (log.isDebugEnabled())
        	if (source != null)
                log.debug("accept: " + target.getPackage().qualifiedName + "<-" + source.getPackage().qualifiedName);
        	else
                log.debug("accept: " + target.getPackage().qualifiedName);
        ...
        {   
            if (log.isDebugEnabled())
                log.debug("ignoring, " + target.getPackage().qualifiedName);
            return;
        }           
