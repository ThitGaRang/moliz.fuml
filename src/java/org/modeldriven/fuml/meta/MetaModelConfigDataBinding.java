...
public class MetaModelConfigDataBinding implements DataBinding {

    private static Log log = LogFactory.getLog(MetaModelConfigDataBinding.class);
    public static String FILENAME_SCHEMA_CHAIN_ROOT = "ModelConfig.xsd";

    // just classes in the same package where can find the above respective
    // schema files via Class.getResource*
    public static Class RESOURCE_CLASS = MetaModelConfigDataBinding.class;

    private ValidatingUnmarshaler validatingUnmarshaler;

    public static Class[] FACTORIES = { org.modeldriven.fuml.meta.config.ObjectFactory.class, };

    private MetaModelConfigDataBinding() {
    }

    public MetaModelConfigDataBinding(BindingValidationEventHandler validationEventHandler)
