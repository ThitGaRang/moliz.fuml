 ...
 * 
 * <em><b>org::modeldriven::fuml::library::unlimitednaturalfunctions::UnlimitedNaturalLessThanFunctionBehaviorExecution</b></em>
 * '. <!-- end-user-doc -->
 * <p>
 * The following features are implemented:
 * <ul>
 * <li>{@link UnlimitedNaturalLessThanFunctionBehaviorExecution#doUnlimitedNaturalFunction <em>
 * doIntegerFunction</em>}</li>
 * <li>{@link UnlimitedNaturalLessThanFunctionBehaviorExecution#new_ <em>new_</em>}</li>
 * </ul>
 * </p>
 * 
 * @generated
...
public class UnlimitedNaturalLessThanFunctionBehaviorExecution extends
    ...
    public Value doUnlimitedNaturalFunction(UnlimitedNatural un1, UnlimitedNatural un2) {
    	...
    	if (un1.naturalValue == -1 && un2.naturalValue == -1) {
    		// Both are the same value, so lessThan function is false
    		bv.value = false;
    	} else if (un2.naturalValue == -1) {
    		// Second argument is highest possible value, so lessThan function is true
    		bv.value = true;
    	} else if (un1.naturalValue == -1) {
    		// First argument is highest possible value, so lessThan function is false
    		bv.value = false;
    	} else {
        	// Neither argument is highest possible value, so perform regular lessThan function
    	...
    	Debug.println("[doBody] Unlimited Natural Less Than results = " + bv.value);
    	return bv;
    }
