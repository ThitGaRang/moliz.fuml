...
public class ValidatingUnmarshaler extends DataBindingSupport {
	    ...
	    throws JAXBException, SAXException
	{
	    ...
	    this.unmarshaler = createUnmarshaler(stream, context, this.handler,
	    		this.resolver);  
	    this.unmarshaler = createUnmarshaler(stream, context);  
	    this.unmarshaler.setEventHandler(validationEventHandler);
	}
    
    public ValidatingUnmarshaler(URL url, JAXBContext context, Handler handler, 
    		Resolver resolver) 
        throws JAXBException, SAXException
    {
    	super(context);
	    ...
	    this.unmarshaler = createUnmarshaler(url, context, this.handler,
	    		this.resolver);  
	}
    ...
    /**
     * Creates an unmarshaler using the given factories and URL.
	 * Loads only the given (subclass) schema as this is the "root" schema and it should include 
	 * any other schema resources it needs, and so on. Note all included schemas MUST
     * be found at the same class level as the root schema. 

     * @param url the Schema URL
     * @param context the SAXB context
     * @param handler the SAX handler
     * @param resolver the SAX resolver
     * @return the unmarshaler
     * @throws JAXBException
     * @throws SAXException
     */
    private Unmarshaller createUnmarshaler(URL url, JAXBContext context, 
    ...
    /**
     * Creates an unmarshaler using the given factories and sream.
	 * Loads only the given (subclass) schema as this is the "root" schema and it should include 
	 * any other schema resources it needs, and so on. Note all included schemas MUST
     * be found at the same class level as the root schema. 

     * @param stream the Schema stream
     * @param context the SAXB context
     * @param handler the SAX handler
     * @param resolver the SAX resolver
     * @return the unmarshaler
